...
describe('orderCreationApi', () => {
  ...
  it('creates and confirms a draft with an idempotency key', async () => {
    ...
    await expect(orderCreationApi.createDraft({ cityId: 7, pointId: 4, customerId: 9, typeOrder: 1, comment: 'Позвонить за час', paymentType: 1, sdacha: 5000, fakeDom: true })).resolves.toMatchObject({ id: 44, status: 'draft' });
    ...
    expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toMatchObject({ comment: 'Позвонить за час', payment_type: 1, sdacha: 5000, fake_dom: true });
  });
