...
- сохранённое время «Ко времени» преобразуется в формат `Y-m-d H:i` и передаётся
  в draft API; доступные интервалы должны загружаться через `preorder-slots`;
- демонстрационный список «Есть кафе НА СТОПЕ» удалён из runtime до появления
  защищённого API-контракта, чтобы оператор не видел выдуманные зоны и сроки.
- каталог «Соусы» и приборы в order-new теперь используют позиции текущего API
  каталога; старые mock-идентификаторы больше не могут попасть в корзину.
  Выбор связанных модификаторов для блюд остаётся отдельным контрактным шагом.
- Header, каталог и доставка используют общий TanStack Query-кэш городов и
  единое разрешение `city_id`, поэтому проверки клиента, адреса и каталога не
  расходятся при переключении города.
